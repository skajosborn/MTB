...
export function Navbar() {
  ...
  const menuItems: RadialMenuItem[] = trails.map(trail => ({
    id: trail.name.toLowerCase().replace(/\s+/g, '-'),
    label: trail.name,
    onClick: () => router.push(`/trails/${trail.name.toLowerCase().replace(/\s+/g, '-')}`),
  }));
                  ...
                  <div
                    ...
                    onMouseEnter={() => {
                      ...
                      if (rect) {
                        ...
                        setMenuPosition({
                          x: rect.left + rect.width / 2 + window.scrollX - 500,
                          y: rect.bottom + window.scrollY - - 100 + MENU_SIZE / 2,
                        });
                        setMenuOpen(true);
                      }
                    }}
